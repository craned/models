...
class BaseTest(tf.test.TestCase):
  ...
  def _construct_and_save_reference_files(
    ...
    with self.test_session(graph=graph) as sess:
      ...
      with open(os.path.join(data_dir, "tf_version.txt"), "wt") as f:
        f.write(tf_version)
  ...
  def _evaluate_test_case(self, name, graph, ops_to_eval, correctness_function):
    ...
    tf_version = tf.__version__
    with open(os.path.join(data_dir, "tf_version.txt"), "rt") as f:
      tf_version_reference = f.read()

    tf_version_comparison = ""
    if tf_version != tf_version_reference:
      tf_version_comparison = (
          "Test was built using:     {}\nLocal TensorFlow version: {}"
          .format(tf_version_reference, tf_version)
      )
